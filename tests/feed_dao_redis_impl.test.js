...
test.only(`${testSuiteName}: insert and read back from global stream`, async () => {
  await insertAndReadBackFromStream();
});

// This test is for Challenge #6.
test.only(`${testSuiteName}: read stream for site that does not exist`, async () => {
...
// This test is for Challenge #6.
test.only(`${testSuiteName}: insert and read back from site specific stream`, async () => {
